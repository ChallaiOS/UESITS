...
public class SpecialContentActivity extends AppCompatActivity implements View.OnTouchListener,GestureDetector.OnGestureListener {
...
//        Boolean isFirstRun = getSharedPreferences("PREFERENCE3", MODE_PRIVATE)
//                .getBoolean("isFirstRun33", true);
//
//        if (isFirstRun) {
//
//            Toast toast =  Toast.makeText(SpecialContentActivity.this, "Tap on title for song sharing", Toast.LENGTH_LONG);
...
//                .putBoolean("isFirstRun", false).commit();
        ...
        textView = findViewById(R.id.textContent4);
...
//        textView2 = (TextView) findViewById(R.id.titleIDLabel);
        ...
        this.gestureDetector = new GestureDetector(SpecialContentActivity.this,this);

        Intent i = getIntent();
        Bundle bundle = i.getExtras();
        itemModel user = (itemModel) bundle.getSerializable("user4");

        currentID = Integer.valueOf(user.getLocal_categorycount().toString());
...
//        }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
                ...
                if(Math.abs(valueX)>MIN_DISTANCE){
                    if (x2>x1){
                        ...
                        if (currentID >1){
                            ...
                            itemModel str = MySingleTon.getInstance().arrayList4.get(currentID - 1);
                            textView.setText(str.getLocal_text());
                            sharecumTitle.setText(str.getLocal_id() + "." + str.getLocal_title());

                        }
                        else{
                            currentID = MySingleTon.getInstance().arrayList4.size();
                            itemModel str = MySingleTon.getInstance().arrayList4.get(currentID - 1);
                            textView.setText(str.getLocal_text());
                            sharecumTitle.setText(str.getLocal_id() + "." + str.getLocal_title());
                        ...
                        this.textView.bringPointIntoView(0);
                        if (currentID > 0){
                            if (currentID == MySingleTon.getInstance().arrayList4.size()) {
                                currentID = 1;
                            }
                            else{
                                currentID = currentID + 1;
        ...
        if (event.getPointerCount() == 2) {
            TextView viewById = (TextView) findViewById(R.id.textContent4);
